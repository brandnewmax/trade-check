...
export function formatIntelAsBriefing(intel) {
  ...
  const mp = intel.maps || {}
  lines.push('## 6. 实地核验(Google Maps)')
  lines.push(statusLine(mp))
  if (mp.status === 'ok') {
    lines.push(`- 查询:\`${mp.query}\``)
    lines.push(`- 结果:${mp.hasRecord ? `✓ 找到 ${mp.resultCount} 个地点` : '✗ 未在 Google Maps 找到地点(可能是新公司、未收录,或地址造假)'}`)
    if (mp.hasRecord) {
      mp.places.forEach((p, i) => {
        const bits = []
        if (p.address) bits.push(`地址:${p.address}`)
        if (p.category) bits.push(`类型:${p.category}`)
        if (p.rating != null) bits.push(`评分:${p.rating}${p.ratingCount ? ` (${p.ratingCount} 评)` : ''}`)
        if (p.website) bits.push(`官网:${p.website}`)
        if (p.phoneNumber) bits.push(`电话:${p.phoneNumber}`)
        lines.push(`  ${i + 1}. ${p.title || '(无标题)'}`)
  ...
  const ph = intel.phone || {}
  lines.push('## 8. 发件方电话核验')
  lines.push(statusLine(ph))
  if (ph.status === 'ok') {
    lines.push(`- 查询:\`${ph.query}\``)
    lines.push(`- 结果:${ph.hitCount > 0 ? `✓ 找到 ${ph.hitCount} 条公开记录` : '✗ 未发现公开记录'}`)
    if (ph.hitCount > 0) lines.push(renderResults(ph.hits))
  }
  lines.push('')
  ...
  lines.push('## 10. 发件方建站时间(Wayback Machine)')
