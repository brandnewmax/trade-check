...

{
  ...
  "address": 发件方完整办公/工厂地址(街道+城市+国家,原文照抄)或 null,
...
- 拿不准的字段填 null,绝不猜测
- **companyUrl 提取规则**:
  - 询盘正文 / 邮件签名 / 名片 / 图片中任何出现的 http://、https:// 或 www. 开头的网址,几乎总是发件方的网址,请优先抽取
  - "我的网站是 X"、"Please visit X"、"website: X"、"check our site X" 这类句式后面的网址必抽
  - 排除明显的社交媒体/搜索引擎/市场平台域名(facebook.com、linkedin.com、alibaba.com、amazon.com 等),这些不是发件方官网
  - 不要根据公司名凭空编造网址
- 电话统一去掉空格和横杠
- 如果图片是名片,按印刷顺序逐字读取
...
export async function getGlobalSettings() {
  ...
  return {
    baseUrl: s.baseUrl || '',
    protocol: s.protocol || 'openai',
    activePrompt: s.activePrompt === 'B' ? 'B' : 'A',
    ...
    extractionModelVision: s.extractionModelVision || '',
    extractionBaseUrl: s.extractionBaseUrl || '',
    extractionApiKey: s.extractionApiKey || '',
    extractionProtocol: s.extractionProtocol || '',
...
// Pure validator for the extraction-related subset of global settings. Used
// by the /api/settings POST route before persisting. Returns null on success,
// or the first user-facing error message (zh-CN) when something is invalid.
//
// Validation order:
//   1. extractionModel non-empty
//   2. extractionModelVision non-empty
//   3. (extractionBaseUrl, extractionApiKey) are atomic — both filled or both blank
export function validateGlobalExtractionSettings(data) {
  const s = (v) => (typeof v === 'string' ? v.trim() : '')
  const model = s(data?.extractionModel)
  const modelVision = s(data?.extractionModelVision)
  const baseUrl = s(data?.extractionBaseUrl)
  const apiKey = s(data?.extractionApiKey)

  if (!model) return '请填写:抽取模型(无图片)'
  if (!modelVision) return '请填写:抽取模型(有图片)'
  if (baseUrl && !apiKey) return '填写独立抽取端点时,Base URL 和 API Key 必须同时填写(缺 API Key)'
  if (apiKey && !baseUrl) return '填写独立抽取端点时,Base URL 和 API Key 必须同时填写(缺 Base URL)'
  return null
}
...
// Fields returned in list view. Excludes the two largest blobs:
//   - `intel`  (tens of KB — search results + snippets)
//   - `result` (~3-8 KB — full LLM markdown report)
// Together these were ~95% of a 50-row history payload. Both are
// fetched on-demand via getQueryById when a card is clicked.
const LIST_FIELDS = [
  'userEmail',
  'url',
  'channel',
...
export async function getQueries(role, email, page = 0, limit = 50) {
  ...
  ids.forEach(id => pipeline.hmget(id, ...LIST_FIELDS))
  const rows = await pipeline.exec()
  return rows
    .map((row, i) => {
      if (!row) return null
      const obj = { id: ids[i] }
      LIST_FIELDS.forEach((f, idx) => {
        if (row[f] !== undefined && row[f] !== null) obj[f] = row[f]
      })
      return obj
    })
    .filter(Boolean)
...
export async function getQueryById(id, role, email) {
  if (!id || typeof id !== 'string' || !id.startsWith('query:')) return null
  const record = await kv.hgetall(id)
  if (!record) return null
  // Non-admins can only read their own records
  if (role !== 'admin' && record.userEmail !== email) return null
  return { id, ...record }
}
