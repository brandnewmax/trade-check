...
SnapCheckAlgo 是一个独立部署的外贸询盘背景调查 API 服务——给定一段询盘文本 + 卖家自画像，返回结构化风险评估（四维评分 + 风险等级）、发件方实体、以及 8 路 OSINT 情报（LinkedIn、Panjiva 海关记录、Wayback 建站时间等）。同时提供一个独立的**卖家公司资料生成**端点，用于 SN 平台用户首次入驻时自动构建公司画像。
...
| 端点 | 用途 | 典型耗时 | 协议 |
|---|---|---|---|
| `POST /api/v1/analyze` | 分析一封询盘的可信度（本文档主体） | 离线 15–30s / 联网 60–120s | SSE |
| `POST /api/v1/profile` | 从卖家网址生成一份 2000–3000 字公司资料报告（新，§10） | 20–80s | SSE |

两个端点**共用同一个 `SERVICE_API_KEY`** 和同一套 SSE/心跳/鉴权契约，学会一个就学会了两个。
...
curl -N -X POST https://snap-check-algo.vercel.app/api/v1/analyze \
  ...
  --max-time 300 \
  -d '{
    "request_id": "req_sn_abc123",
    "inquiry_text": "Hello, I am Sarah from Lagos Trading. Interested in bulk LED, 5000 units, CIF Lagos. sarah@lagostrading.com",
    "company_profile": "Shenzhen Bright LED Co., Ltd. — LED display manufacturer, website https://shenzhen-bright-led.example.com",
    "enable_intel": true,
    "scan_mode": "online"
  }'
```
...
> **旧字段兼容**: 仍然接受 `{inquiry, company:{…}, images, options:{enable_intel}}` 的老形状。新字段名优先，老字段名 fallback。新集成请直接用新形状。详见 §2.1。
...
{
  "request_id": "<追踪 ID，可选>",
  "inquiry_text": "<询盘原文，必填>",
  "company_profile": "<卖家公司资料字符串或结构化对象>",
  "inquiry_images": [
...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `request_id` | string | × | SN 端生成的追踪 ID（例如 `req_<uuid>`）。若省略，服务端会生成。**仅用于追踪**，不参与业务逻辑 |
| `inquiry_text` | string | ✓ | 询盘原文。空字符串 → 400 |
| `company_profile` | string \| object | × | 卖家（我方）公司资料。可以是字符串（推荐：直接传 `/api/v1/profile` 生成的 `profile_report`），也可以是 `{name, website, intro, industry, product_lines}` 结构化对象 |
| `inquiry_images` | Array | × | 最多 4 张图片（名片/邮件截图等），每项支持 `url` 或 `base64` 二选一 |
| `enable_intel` | boolean | × | 默认 `false`。详见 §3 |
| `scan_mode` | `"online" \| "offline"` | × | 显式声明模式。缺省时由 `enable_intel` 推导 |

**旧形状（向下兼容）**: 仍接受 `{inquiry, company:{name, website, intro, industry, product_lines}, images, options:{enable_intel}}`，语义等价。新字段名优先，老字段名作为 fallback。新集成**不要**混用新老字段。

> **`company_profile` 最佳实践**: SN 平台用户首次入驻时调用 `/api/v1/profile`（§10）生成 2000–3000 字 markdown 报告并存入 SN 数据库。后续所有 `/api/v1/analyze` 请求直接把这段 markdown 字符串作为 `company_profile` 传进来——无需再拆字段。
...
| 400 | JSON body 解析失败 / `inquiry_text` 为空 | `{"ok":false,"error":"inquiry is required"}` |
...
- `tokens` 理想情况为 `{prompt: <number>, completion: <number>}`；部分上游 provider 不透传 usage 时可能为 `{prompt: null, completion: null}`
...
| 取值 | 含义 | 典型耗时 | 外部调用 |
|---|---|---|---|
| `true` | 启用 8 路并发 OSINT 情报搜集（Serper + Wayback） | **60–120s** | SerpAPI 消耗 6-8 次搜索额度 |
| `false`（默认） | 只跑主 LLM 分析（用 fallback prompt） | **15–30s** | 无 |

> ⚠️ **默认值变更**：2026-04-17 起 `enable_intel` 默认 `false`（此前默认 `true`）。想要联网必须显式传 `true`。这是 CTO 明确的产品决策——"没主动要情报就别花 SerpAPI 额度"。
...
`scan_mode` 与 `enable_intel` 的关系：两者表达同一维度。如果同时传，`enable_intel` 是源（实际跑不跑情报），`scan_mode` 是标签（观察日志里存 `"online"/"offline"`）。不传 `scan_mode` 时服务端从 `enable_intel` 推导。
...
**SN 后端请自己生成 `request_id`**（例如 `req_<ulid>` 或 `req_sn_<analysis_id>`），在请求体里传给 SnapCheckAlgo。同一个 ID 就会作为观察日志的 key 后缀落库——SN 这端和 Algo 这端天然对齐。

不传也行，服务端会生成 uuid v4 fallback。但那样排查时两边对不上。
...
- **TTL**: 90 天
- **字段**: `request_id`, `timestamp`, `scan_mode`(`online`/`offline`), `input_hash`(SHA-256), `output_summary`, `elapsed_ms`, `enable_intel`, `source`(`sn_platform_api`/`web_frontend`), `status`, `error_code`

> **2026-04-17 变更**：观察日志移除了原先的 `user_id` 字段（`"service"/"web"`），改为 `scan_mode` 做调用模式归类。`source` 保留用于区分来自 SN 平台 API 还是 Web 前端。**Algo 侧不存任何业务级身份**——业务级身份由 SN 后端在自己的数据库里保存。

`/api/v1/profile` 有独立的观察日志 key 前缀 `obs:profile:{YYYYMMDD}:{request_id}`，字段见 §10.5。

排查特定请求：SN 运维把 `request_id` 直接丢给 SnapCheckAlgo 运维，`GET obs:analyze:{YYYYMMDD}:{request_id}` 一把到位。
...
这份指南对应 SnapCheckAlgo 当前生产版本（**2026-04-18 部署**）。

### 2026-04-18 更新要点

- 🆕 新增 `POST /api/v1/profile`（§10），供 SN 平台用户首次入驻生成公司资料
- 🆕 `/api/v1/analyze` 请求体新字段名：`request_id` / `inquiry_text` / `company_profile` / `inquiry_images` / `enable_intel` / `scan_mode`（旧字段向下兼容）
- ⚠️ `enable_intel` 默认值由 `true` 变为 `false`——想要联网必须显式传
- 🆕 SN 端可自己生成 `request_id` 传入，两端对齐排查链路
- 🔧 观察日志字段重构：移除 `user_id`，加 `scan_mode`；`source` 保留
- 🔧 `/api/analyze`（Web 前端路径）修好 token usage 透传，`done` 事件 + 观察日志 `tokens` 字段现在带真实数字
- 🔧 `scores.strategy` 正则放宽，兼容 "综合/战略/策略执行" 等 LLM 输出变体
...
## 10. `POST /api/v1/profile` — 公司资料生成端点

SN 平台用户首次入驻时调用此端点，从用户提供的官网 URL 自动生成 2000–3000 字 markdown 公司资料报告。生成后的报告建议直接存入 SN 的 user profile 表，后续每次调用 `/api/v1/analyze` 时作为 `company_profile` 字符串传入。

### 10.1 Headers

和 `/api/v1/analyze` 完全一样：`Authorization: Bearer <SERVICE_API_KEY>` + `Content-Type: application/json`。响应是 `text/event-stream` SSE。
...
```json
{
  ...
  "supplementary_info": "<用户补充的产品资料、文案、公司说明，可选>"
}
```

| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `request_id` | string | × | 追踪 ID；同 `/analyze`，不传时服务端生成 |
| `website_url` | string | ✓ | 合法 http/https URL。非法格式 → 400 |
| `supplementary_info` | string | × | 用户手动输入的补充文案（比如"我们主做 LED 显示屏，客户主要在东南亚"），帮助 LLM 在网站内容不够时兜底 |

### 10.3 处理流程（4 步）

```
1. fetch_website          ── 浏览器 UA 直抓 website_url (15s 超时，2MB 上限)
   │
   ├─ 成功：纯文本 ≥ 200 字
   │        ↓
   │       LLM 生成                  → source: "direct_fetch", website_accessible: true
   │
   └─ 失败：403 / 429 / 503 / Cloudflare challenge / 纯文本 < 200 字
          ↓
2. fetch_fallback_serp    ── SerpAPI `site:<域名>` 搜前 10 条，title + snippet 聚合
   │
   ├─ 成功：聚合 ≥ 200 字 → LLM 生成  → source: "serp_fallback", website_accessible: false
   │
   └─ 失败：error 事件   code = "fetch_blocked" | "content_too_short"
```

**stage 枚举**（progress 事件的 `stage` 字段）：
`queued` → `fetch_website` → （可选）`fetch_fallback_serp` → `llm_generate`

心跳：和 `/analyze` 一样，每 8s 一条 `progress`。

### 10.4 `done` 事件 data 契约

```json
{
  "ok": true,
  "data": {
    "profile_report": "<2000-3000 字 markdown 公司资料报告>",
    "source": "direct_fetch",
    "website_accessible": true,
    "model": "claude-sonnet-4-6",
    "tokens": { "prompt": 3200, "completion": 2100 },
    "elapsed_ms": 28000
  }
}
```
...
**字段契约：**
- `profile_report` — markdown 中文，约 2000–3000 字。报告结构由 LLM 生成，包含：公司概况 / 主营产品 / 目标客户画像 / 业务模式 / 竞争优势 / 潜在合作方向。某个维度信息不足时 LLM 会注明"信息不足"而不是编造
- `source` — `"direct_fetch"` (官网可抓) 或 `"serp_fallback"` (靠 Google 索引拼凑)
- `website_accessible` — `true`/`false`。`false` 时 SN 前端可以给用户一个"你的官网对搜索爬虫不友好，可能影响询盘调查的质量"的温和提示
- `tokens` — 和 `/analyze` 同构

### 10.5 `error` 事件 code 枚举

| code | 场景 | SN 建议动作 |
|---|---|---|
| `invalid_url` | URL 格式非法（走 HTTP 400，不是 SSE） | 前置校验用户输入 |
| `fetch_not_found` | 404 或 DNS 失败 | 提示用户核对网址 |
| `fetch_blocked` | 直抓 + SerpAPI 都拿不到足够内容 | 让用户手动贴大段 `supplementary_info` 重试 |
| `content_too_short` | SerpAPI 聚合后仍 < 200 字 | 同上 |
| `llm_error` | LLM 调用失败 | 指数退避重试 1–2 次 |
| `config` | 服务端配置缺失 | 报警，不要重试 |
| `internal` | 未分类异常 | 带 `request_id` 报警 |

### 10.6 观察日志

每次调用都写入 Upstash Redis：

- **Key**: `obs:profile:{YYYYMMDD}:{request_id}`
- **TTL**: 90 天
- **字段**: `request_id`, `timestamp`, `website_url_hash`(SHA-256，不存原 URL), `source`, `website_accessible`, `tokens`, `elapsed_ms`, `status`, `error_code`
...
SN 用户入驻向导
 ...
 │
 ▼
SN 后端 POST /api/v1/profile
 │
 ▼
成功：profile_report 存入 SN user 表
失败 (fetch_blocked / content_too_short)：
 ├── 显示"无法自动抓取，请手动填写"表单
 └── 后备由用户自己贴 markdown
 │
 ▼
后续每次 /api/v1/analyze 时
  company_profile: <user.profile_report 字符串>
```

SN 前端展示建议：首次抓取是一个 SSE 进度条 UI（20–80s），`fetch_fallback_serp` 阶段可以显示"官网直接抓取失败，正在通过搜索引擎构建资料...",给用户预期管理。

### 10.8 Node.js 调用示例

```js
async function generateCompanyProfile({ requestId, websiteUrl, supplementaryInfo }) {
  const res = await fetch('https://snap-check-algo.vercel.app/api/v1/profile', {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${process.env.SNAPCHECK_SERVICE_API_KEY}`,
      'Content-Type': 'application/json',
  ...
  // SSE 解析与 /analyze 完全同构，见 §5.1
  // 复用同一个 consumeSSE helper，收到 done 返回 data
  const done = await consumeSSE(res)
  return done.data  // { profile_report, source, website_accessible, model, tokens, elapsed_ms }
}
```
